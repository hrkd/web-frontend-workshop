...
今回は皆さんからリクエストのあったフレームワーク（React / Astro / Bootstrap / Tailwind）について紹介していきます。特にReactなどのフレームワークはマークアップとプログラミングの合流をスムーズにしてくれるはずです。
...
Astroは2021年、Fred K. Schottらによって開発されました。ブログやドキュメントサイトなど、**コンテンツ重視のWebサイト** を高速に構築するために生まれました。

#### SPAの課題

ReactやVueで作られたSPA（Single Page Application）は、すべてのUIをJavaScriptで構築します。これは高度なインタラクションには適していますが、ブログや企業サイトには過剰なことも。
...
Astroは「アイランドアーキテクチャ」という考え方を採用しています。ページの大部分は静的なHTMLとして配信し、インタラクティブな部分だけをJavaScriptで動かします。
...
必要な部分だけが「島」のようにインタラクティブになります。

#### 複数フレームワークの共存

Astroの大きな特徴は、React、Vue、Svelte、Solidなどを **同じプロジェクトで混在** できることです。
...
必要なタイミングでだけJavaScriptを読み込みます。

#### デフォルトでゼロJS

Astroは **デフォルトでJavaScriptを出力しません**。`client:*` ディレクティブを明示的に指定したコンポーネントだけがJavaScriptを含みます。これにより、非常に高速なページ表示が可能になります。
