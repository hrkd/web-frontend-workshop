...
今回は皆さんからリクエストのあったフレームワーク<br>（React / Astro / Bootstrap / Tailwind）について紹介していきます。

特にReactなどのフレームワークはマークアップとプログラミングの合流を<br>スムーズにしてくれるはずです。
...
### Astro：コンテンツ優先のWebフレームワーク

---

#### 静的サイトジェネレーターの進化（2021年）

Astroは2021年、Fred K. Schottらによって開発されました。

ブログやドキュメントサイトなど、<br>**コンテンツ重視のWebサイト** を高速に構築するために生まれました。

---

#### SPAの課題

ReactやVueで作られたSPA（Single Page Application）は、<br>すべてのUIをJavaScriptで構築します。

これは高度なインタラクションには適していますが、<br>ブログや企業サイトには過剰なことも。

---

- 初期表示が遅い（JavaScriptの読み込み・実行が必要）
- SEOの考慮が必要（SSRなどの対策）
- シンプルなコンテンツサイトには複雑すぎる

---

#### Islands Architecture

Astroは「アイランドアーキテクチャ」という考え方を採用。

ページの大部分は静的なHTMLとして配信し、<br>インタラクティブな部分だけをJavaScriptで動かします。
...
必要な部分だけが「島」のようにインタラクティブになる

---

#### 複数フレームワークの共存

Astroの大きな特徴は、<br>React、Vue、Svelte、Solidなどを **同じプロジェクトで混在** できること。
...
- `client:visible` - 画面に表示されたらハイドレーション
- `client:idle` - ブラウザが暇になったらハイドレーション

必要なタイミングでだけJavaScriptを読み込む

---

#### デフォルトでゼロJS

Astroは **デフォルトでJavaScriptを出力しません**。

`client:*` ディレクティブを明示的に指定した<br>コンポーネントだけがJavaScriptを含みます。

これにより、非常に高速なページ表示が可能に。
...
- コンテンツ重視のサイト向けWebフレームワーク
- デフォルトでJavaScriptを出力しない（高速）
- React/Vue/Svelteなど複数フレームワークを混在可能
- Islands Architectureで必要な部分だけインタラクティブに
...
- ブログ、ドキュメントサイト、企業サイトなどコンテンツ中心のサイト
- ページ表示速度を最優先したい
- 既存のReact/Vueコンポーネントを活用しつつ、静的サイトを作りたい

---

### Bootstrap

- デザイナーがいない、またはデザインに時間をかけられない
- 管理画面や社内ツールなど、見た目の独自性より一貫性が重要

---

### Tailwind

- 既存のCSS設計・命名規則に限界を感じている
- CSSファイルとHTMLを行き来するのがつらい
- チームでスタイルの書き方を統一したい
