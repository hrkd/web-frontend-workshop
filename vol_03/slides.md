...
今回は皆さんからリクエストのあった<br>フレームワークについて紹介していきます。
...
しかし2000年代後半から、Gmail、Googleマップなど、<br>ページ遷移なしでUIが変化する **Webアプリケーション** が登場しました。

---

JavaScriptでDOMを操作して<br>UIを動的に変える必要が出てきました。
...
そこで、ReactコンポーネントとしてUIを提供するライブラリがあります。
...
これらを使うと、コンポーネント内で<br>**手続き型（ロジック）** と **宣言型（JSX）** の<br>両方を乗りこなすことになります。

前回整理した「マークアップとプログラミングの違い」が合流する場所です。
