...
まず前提として、予算と時間があります。極端な話、予算がなくて1日で仕上げなければならないものは、Figmaからアートボードを書き出した1枚画像でも構わないと私は思います。明確なアクセシビリティが要件に含まれるなら、そのような予算やスケジュールにはならないはずです。
...
プログラミングとは「何をするか」手順を記述することだと説明しました。ではソフトウェアエンジニアは日々何をしているのか。それは、ビジネス上の大きな問題を、コードとして書ける大きさまで分解し、プログラミングすることです。
...
async function checkAvailability(eventId, date) {
  // 1. データベースから対象イベントの定員を取得する
  const event = await getEventFromDB(eventId);

  // 2. データベースからその日時の現在の予約数を取得する
  const currentReservations = await getReservationCountFromDB(eventId, date);

  // 3. 予約数が定員未満なら「空きあり」を返す
  return currentReservations < event.capacity;
}
```

分解した3つの手順が、そのままコードになっています。プログラミングとは、こうしてビジネス上の大きな問題を、コードとして書ける大きさまで分解して、手順として記述することです。
...
今回、マークアップとプログラミングは別のスキルだと説明しました。しかし現代のフロントエンド開発では、この2つを同時に扱う必要があります。次回はそれを可能にするフレームワークやライブラリについて、選定基準と合わせて紹介します。
