...
1. [はじめに](#はじめに)
2. [コンピュータの動く3つの場所](#コンピュータの動く3つの場所)
3. [スキルセット分布図](#スキルセット分布図)
4. [ビルドツールの話](#ビルドツールの話)
5. [Webサーバーとランタイムの関係](#webサーバーとランタイムの関係)
6. [ブラウザのサンドボックス](#ブラウザのサンドボックス)
7. [まとめ](#まとめ)
8. [次回予告](#次回予告)
...
現代ではPHP-FPMという仕組みで、PHPプロセスが常駐して待機しています。nginxがリクエストを受けたらPHP-FPMに処理を依頼し、結果を受け取って返します。PHPがApacheやnginxとセットで語られるのは、本番環境ではPHP単体ではなく、専用のWebサーバーと組み合わせて使うのが一般的だからです。（※ 開発用途ではPHP組み込みサーバーも利用可能）
